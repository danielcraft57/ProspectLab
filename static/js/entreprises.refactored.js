    ...
    function renderUXExpertise(analysis) {
        ...
        const findingsHtml = findings.map((f) => `
            <div class="ux-finding" style="border:1px solid #e2e8f0;border-radius:8px;padding:0.75rem 1rem;margin-bottom:0.5rem;">
                ...
                ${f.corpus_best_quote ? `<blockquote style="margin:0.5rem 0 0;padding:0.5rem 0.75rem;border-left:3px solid #94a3b8;color:#64748b;font-size:0.85rem;">« ${esc(f.corpus_best_quote)} »${f.corpus_best_source ? ` <cite>— ${esc(f.corpus_best_source)}</cite>` : ''}</blockquote>` : ''}
                ${f.chapter_title ? `<p style="margin:0.35rem 0 0;font-size:0.8rem;color:#94a3b8;">Ch.${esc(f.chapter)} — ${esc(f.chapter_title)}${f.corpus_docs_scanned ? ` · corpus ${f.corpus_docs_scanned} transcripts` : ''}</p>` : ''}
            </div>
        `).join('');
