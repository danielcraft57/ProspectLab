...
# Sécurité: si le repo local est sale, on n'applique aucun pull automatique.
# Usage:
...
#   GIT_BRANCH=main
#   CRON_SCHEDULE="*/5 * * * *"
#   LOG_FILE=/opt/prospectlab/logs/prospectlab_git_sync.log

PROSPECTLAB_PATH="${PROSPECTLAB_PATH:-/opt/prospectlab}"
GIT_BRANCH="${GIT_BRANCH:-main}"
CRON_SCHEDULE="${CRON_SCHEDULE:-*/5 * * * *}"
LOG_FILE="${LOG_FILE:-/opt/prospectlab/logs/prospectlab_git_sync.log}"
...
CRON_CMD="cd $PROSPECTLAB_PATH && if [ -n \"\$(git status --porcelain)\" ]; then echo \"[\$(date -Iseconds)] Repo dirty, sync skipped\" >> $LOG_FILE; exit 0; fi && git fetch --all --prune && git pull --ff-only origin $GIT_BRANCH >> $LOG_FILE 2>&1"
