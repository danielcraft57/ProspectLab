...
from services.multicanal import ChannelName, MessagePayload, MultiCanalService, XDirectMessageProvider


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Test d'envoi DM X (Twitter) via services.multicanal."
    )
    parser.add_argument(
        "--recipient-id",
        required=True,
        help="ID utilisateur X du destinataire (numerique).",
    )
    parser.add_argument(
        "--body",
        required=True,
...
def main() -> int:
    ...
    payload = MessagePayload(
        ...
        body=args.body,
        metadata={"source": "scripts/tests/test_multicanal_x_dm.py"},
    )
