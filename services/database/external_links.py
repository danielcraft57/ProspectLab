...
class ExternalLinksManager(DatabaseBase):
    ...
    def _migrate_external_link_page_locations_columns(self, cursor) -> None:
        ...
        if not self._table_exists(cursor, 'external_link_page_locations'):
            return
        lat_t = 'DOUBLE PRECISION' if self.is_postgresql() else 'REAL'
        for ddl in (
            ...
            f'ALTER TABLE external_link_page_locations ADD COLUMN latitude {lat_t}',
            f'ALTER TABLE external_link_page_locations ADD COLUMN longitude {lat_t}',
